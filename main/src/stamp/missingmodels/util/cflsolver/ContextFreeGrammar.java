...
public class ContextFreeGrammar {
	...
	public static final class SymbolMap {
		...
		public SymbolMap(SymbolMap other) {
			for(Map.Entry<String,Symbol> symbol : other.symbols.entrySet()) {
				this.symbols.put(symbol.getKey(), symbol.getValue());
			}
			for(Map.Entry<Integer,Symbol> symbol : other.symbolsById.entrySet()) {
				this.symbolsById.put(symbol.getKey(), symbol.getValue());
			}
		...
		private Symbol add(String symbol) {
			if(this.symbols.keySet().contains(symbol)) {
				throw new RuntimeException("Already contains symbol: " + symbol);
			}
			Symbol result = new Symbol(symbol, this.numLabels++);
	...
	public static final class ContextFreeGrammarOpt {
		...
		private ContextFreeGrammarOpt(ContextFreeGrammar c) {
			this.symbols = new SymbolMap(c.symbols);
