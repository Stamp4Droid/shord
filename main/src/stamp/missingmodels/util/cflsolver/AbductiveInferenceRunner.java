...
import stamp.missingmodels.util.cflsolver.Graph.EdgeTransformer;
...
public class AbductiveInferenceRunner {
	...
	private static Graph computeTransitiveClosure(ContextFreeGrammarOpt c, Graph g, boolean shord) {
		long time = System.currentTimeMillis();
		System.out.println("Computing transitive closure");
		
		Graph gbar = g.transform(new ReachabilitySolver(c));
	...
	private static Map<EdgeStruct,Boolean> runAbductiveInference(AbductiveInferenceHelper h, ContextFreeGrammarOpt c, Graph gbart, Graph gcur, boolean shord) throws LpSolveException {
	...
	private static void recomputeTransitiveClosure(ContextFreeGrammarOpt c, Graph gbar, TypeFilter t, Set<Edge> baseEdges, final Map<EdgeStruct,Boolean> result) {
		...
		EdgeTransformer gtNew = new EdgeTransformer(gbar.getSymbols()) {
			@Override
		...
		Graph gnewBar = gnew.transform(new ReachabilitySolver(c));
	...
	private static Graph removeResultingEdges(Graph g, final Map<EdgeStruct,Boolean> result) {
		EdgeTransformer gt = new EdgeTransformer(g.getSymbols()) {
			@Override
	...
	public static MultivalueMap<EdgeStruct,Integer> runInference(AbductiveInferenceHelper h, ContextFreeGrammarOpt c, Graph g, boolean shord, int numCuts) throws LpSolveException {
		Graph gcur = g;
		MultivalueMap<EdgeStruct,Integer> allResults = new MultivalueMap<EdgeStruct,Integer>();
		for(int i=0; i<numCuts; i++) {
			// STEP 1: Run reachability solver
			Graph gbar = computeTransitiveClosure(c, gcur, shord);
			...
			Map<EdgeStruct,Boolean> result = runAbductiveInference(h, c, gbar, gcur, shord);
	...
	public static MultivalueMap<EdgeStruct,Integer> runInference(ContextFreeGrammarOpt c, Graph g, boolean shord, int numCuts) throws LpSolveException {
