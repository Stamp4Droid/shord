...
import stamp.missingmodels.util.cflsolver.ContextFreeGrammar.AuxProduction;
...
import stamp.missingmodels.util.cflsolver.Edge.Field;
import stamp.missingmodels.util.cflsolver.LinearProgram.Coefficient;
import stamp.missingmodels.util.cflsolver.LinearProgram.ConstraintType;
import stamp.missingmodels.util.cflsolver.LinearProgram.LinearProgramResult;
import stamp.missingmodels.util.cflsolver.LinearProgram.ObjectiveType;
	...
	private void processProduction(BinaryProduction binaryProduction, Edge target) {
		...
		for(Edge firstInput : potentialFirstInputs) {
			...
			for(Edge secondInput : potentialSecondInputs) {
				...
				// only add production if edge data are equal
				if(Field.produce(binaryProduction.ignoreFields, firstInput.field, secondInput.field) != target.field) {
					continue;
				}
				/*
