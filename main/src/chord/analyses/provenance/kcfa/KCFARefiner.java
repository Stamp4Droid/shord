...
public class KCFARefiner extends JavaAnalysis {
	...
	public void run() {
		try {
			debugPW = new PrintWriter(new File(Config.v().outDirName + File.separator + "debug.txt"));
			statPW = new PrintWriter(new File(Config.v().outDirName+File.separator+"stat.txt"));
		...
		String kinitConfig = "datalog/chord-provenance/inst/kcfa-bit-init-dlog_XZ89_.config";
		String kcfaConfig = "datalog/chord-provenance/inst/pro-cspa-kcfa-dlog_XZ89_.config";
		...
		for (int i = 0; i < domH.size(); i++) {
			AllocNode H = (AllocNode) domH.get(i);
			OKRel.add(H,0);
		}
		OKRel.save();	
...
//				gen = current; // some recover, not entirely necessary
	...
	private void setHK(AllocNode q, int k){
		HKRel.add(q,k);
		for(int i = 0; i <= k; i++){
			allowHRel.add(q,i);
		}
