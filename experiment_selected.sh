#!/bin/bash

echo ./stamp analyze ../stamptest/MissingModels/andRoc-rev418.apk
./stamp analyze ../stamptest/MissingModels/andRoc-rev418.apk
echo ./stamp analyze ../stamptest/MissingModels/angulo-1.2.apk
./stamp analyze ../stamptest/MissingModels/angulo-1.2.apk
echo ./stamp analyze ../stamptest/MissingModels/RemoteDroid-v1.4.apk
./stamp analyze ../stamptest/MissingModels/RemoteDroid-v1.4.apk
echo ./stamp analyze ../stamptest/MissingModels/tomdroid-0.7.3.apk
./stamp analyze ../stamptest/MissingModels/tomdroid-0.7.3.apk
echo ./stamp analyze ../stamptest/MissingModels/TUIOdroid.apk
./stamp analyze ../stamptest/MissingModels/TUIOdroid.apk
echo ./stamp analyze ../stamptest/MissingModels/yaaic-1.0.apk
./stamp analyze ../stamptest/MissingModels/yaaic-1.0.apk

echo ./stamp analyze ../stamptest/DarpaApps/0_Sip2Peer
./stamp analyze ../stamptest/DarpaApps/0_Sip2Peer
echo ./stamp analyze ../stamptest/DarpaApps/0_TheKeyboard
./stamp analyze ../stamptest/DarpaApps/0_TheKeyboard
echo ./stamp analyze ../stamptest/DarpaApps/1A_AndroidGame
./stamp analyze ../stamptest/DarpaApps/1A_AndroidGame
echo ./stamp analyze ../stamptest/DarpaApps/1A_Butane
./stamp analyze ../stamptest/DarpaApps/1A_Butane
echo ./stamp analyze ../stamptest/DarpaApps/1A_ConnectBot
./stamp analyze ../stamptest/DarpaApps/1A_ConnectBot
echo ./stamp analyze ../stamptest/DarpaApps/1A_TextSecure
./stamp analyze ../stamptest/DarpaApps/1A_TextSecure
echo ./stamp analyze ../stamptest/DarpaApps/1B_AndBible
./stamp analyze ../stamptest/DarpaApps/1B_AndBible
echo ./stamp analyze ../stamptest/DarpaApps/1B_OpenGPSTracker
./stamp analyze ../stamptest/DarpaApps/1B_OpenGPSTracker
echo ./stamp analyze ../stamptest/DarpaApps/1C_tomdroid
./stamp analyze ../stamptest/DarpaApps/1C_tomdroid
echo ./stamp analyze ../stamptest/DarpaApps/2A_GeoTagger
./stamp analyze ../stamptest/DarpaApps/2A_GeoTagger
echo ./stamp analyze ../stamptest/DarpaApps/2B_fillinfun
./stamp analyze ../stamptest/DarpaApps/2B_fillinfun
echo ./stamp analyze ../stamptest/DarpaApps/2B_SMSBot
./stamp analyze ../stamptest/DarpaApps/2B_SMSBot
