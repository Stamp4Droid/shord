 ...
 (
   select node_id as recv_id, tgtId as serviceId, tgt_node_id as activity_id from (
     /*receiver a will launch b*/
       ...
       (select node_id, e.tgt_node_id as tgtId from intentFilter ift,  edge e where ift.iccg_id=? and (ift.name like '%BOOT_COMPLETE%') and e.src_node_id=ift. node_id)  as tmp,
