...
/*recv1->recv2*/
(
select tmp.src_node_id as recv1Id, tmp.tgt_node_id as recv2Id from (select * from edge e1, node recv1, node recv2 where e1.iccg_id=? and recv1.iccg_id=? and recv2.iccg_id=?  and recv1.type='receiver' and recv2.type='receiver' and e1.src_node_id=recv1.id and e1.tgt_node_id=
recv2.id) as tmp left join intentFilter as ift where ift.node_id = tmp.src_node_id and (ift.name like '%BOOT_COMPLETED%')
) as tmp2, edge as e2, edge as e3 where  e2.iccg_id=? and e3.iccg_id=? and e2.src_node_id=tmp2.recv2Id and e3.tgt_node_id=tmp2.recv2Id and e3.src_node_id=e2.tgt_node_id 
