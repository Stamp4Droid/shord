...
def lang_pair_to_str((c,o)):
    return '%s | %s' % (c.reverse(), o)
...
def solve(base_lang, init_xforms, verbose=False):
    ...
    while True:
        ...
        log('Level 0')
        log('    ' + lang_pair_to_str(base_lang))
        log('Level 1')
        lvl1_lang = base_lang
        for t1 in xforms:
            (c1,o1) = t1.apply(base_lang)
            log('    ' + lang_pair_to_str((c1,o1)))
            log('        %s' % t1)
            (l1c,l1o) = lvl1_lang
            lvl1_lang = (l1c | c1, l1o | o1)
        log('    ' + lang_pair_to_str(lvl1_lang))
        log('Level 2')
        for t2 in xforms:
            (c2,o2) = t2.apply(lvl1_lang)
            (l1c,l1o) = lvl1_lang
            if c2 <= l1c and o2 <= l1o:
                continue
            log('    ' + lang_pair_to_str((c2,o2)))
            log('        %s' % t2)
            for t1 in xforms:
                ts = t1 >> t2
                log('        compose after %s' % t1)
                for t in ts:
                    (c,o) = t.apply(base_lang)
